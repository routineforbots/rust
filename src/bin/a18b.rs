...
fn authorize(
    ...
    // 1. We use the `?` operator to extract the `Database` value, propagating a `Result::Err` value to the caller
    let db = Database::connect()?;

    /* 2. Alternatively we can handle errors and oks as (option with ? has the same logic):
    ...
    */
    ...
    // 3. Or to get just the result we can use 
    // let db = Database::connect().unwrap() - but that will not handle the error

    let requester = db.find_employee(employee_name)?;

    //println!("Extracting user's permissions...");
    let requester_card = db.get_keycard(&requester)?;

    if requester_card.access_level >= location.required_access_level() {
        //println!("Access granted to level: {:?}", &location);
        return Ok(AuthorizationStatus::Allow);
        
    } else {
        //println!("Not authorized to any levels");
